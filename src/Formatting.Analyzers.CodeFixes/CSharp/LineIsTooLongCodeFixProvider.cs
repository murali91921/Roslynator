...
namespace Roslynator.Formatting.CodeFixes
{
    ...
    internal class LineIsTooLongCodeFixProvider : BaseCodeFixProvider
    {
        ...
        public sealed override async Task RegisterCodeFixesAsync(CodeFixContext context)
        {
            ...
            var wrapLineAnalysis = new WrapLineAnalysis(document, span, maxLength);
            ...
            while (position >= span.Start)
            {
                ...
                for (; node?.SpanStart >= span.Start; node = node.Parent)
                {
                    ...
                    //if (wrapLineAnalysis.TryGetNode(kind, out SyntaxNode node2)
                    //    && object.ReferenceEquals(node, node2))
                    //{
                    //    continue;
                    //}

                    if (wrapLineAnalysis.ProcessNode(node))
                        break;
            ...
            SyntaxNode nodeToFix = wrapLineAnalysis.GetNodeToFix();
