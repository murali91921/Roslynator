...
namespace Roslynator.CommandLine
{
    ...
    internal static class Program
    {
        ...
        {
            string[] all = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.all.txt");
            string[] core = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.core.dictionary");
            string[] core2 = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.core2.dictionary");
            string[] derived = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.derived.dictionary");
            string[] it = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.it.dictionary");

            if (File.Exists(@"E:\Projects\Roslynator\src\CommandLine\a.dictionary"))
            {
                var a = File.ReadAllLines(@"E:\Projects\Roslynator\src\CommandLine\a.dictionary");

                File.WriteAllText(
                    @"E:\Projects\Roslynator\src\CommandLine\a.dictionary",
                    string.Join(Environment.NewLine, a.Except(core, StringComparer.OrdinalIgnoreCase).Except(core2, StringComparer.OrdinalIgnoreCase).Except(it, StringComparer.OrdinalIgnoreCase).Distinct().OrderBy(f => f, StringComparer.InvariantCulture).ToArray()));
            }

            File.WriteAllText(
                @"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.core2.dictionary",
                string.Join(Environment.NewLine, core2.Except(core, StringComparer.OrdinalIgnoreCase).Except(it, StringComparer.OrdinalIgnoreCase).Select(f => f.ToLower()).Distinct().OrderBy(f => f, StringComparer.InvariantCulture).ToArray()));

            File.WriteAllText(
                @"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.custom.dictionary",
                string.Join(Environment.NewLine, it.Except(core, StringComparer.OrdinalIgnoreCase).Except(core2, StringComparer.OrdinalIgnoreCase).Select(f => f.ToLower()).Distinct().OrderBy(f => f, StringComparer.InvariantCulture).ToArray()));

            var coreExcept = core.Except(all, StringComparer.OrdinalIgnoreCase).ToArray();
            var core2Except = core2.Except(all, StringComparer.OrdinalIgnoreCase).ToArray();
            var derivedExcept = derived.Except(all, StringComparer.OrdinalIgnoreCase).ToArray();

            Debug.Assert(coreExcept.Length == 0, coreExcept.Length.ToString());
            Debug.Assert(core2Except.Length == 0, core2Except.Length.ToString());
            Debug.Assert(derivedExcept.Length == 0, derivedExcept.Length.ToString());
           ...
           var gen = items
              .Except(core, StringComparer.OrdinalIgnoreCase)
              .Except(core2, StringComparer.OrdinalIgnoreCase)
              .Except(derived, StringComparer.OrdinalIgnoreCase)
              .Except(it, StringComparer.OrdinalIgnoreCase)
              .Intersect(all, StringComparer.OrdinalIgnoreCase)
              .Select(f => f.ToLower())
              .Distinct()
            ...
            File.WriteAllText(
                @"E:\Projects\Roslynator\src\CommandLine\roslynator.spelling.derived.generated",
                string.Join(Environment.NewLine, gen));

            void GenerateWords()
            {
                foreach (var item in core.Union(core2, StringComparer.OrdinalIgnoreCase).Union(items, StringComparer.OrdinalIgnoreCase).ToArray())
                {
                            ...
                            case "s":
                                {
                                    ...
                                    {
                                        items.Add(item.Remove(item.Length - 1) + "ies");
                                    }
                                    else if (item.EndsWith("x"))
                                    {
                                        items.Add(item + "es");
                                    }
                                    else
                            ...
                            case "ness":
                                {
                                    if (item.EndsWith("y"))
                                    {
                                        items.Add(item.Remove(item.Length - 1) + "iness");
                                    }
                                    else
                                    {
                                        items.Add(item + suffix);
                            ...
                            case "er":
                                {
                                    if (item.EndsWith("y"))
                                    {
                                        items.Add(item.Remove(item.Length - 1) + "ier");
                                    }
                                    else
                                    {
                                        items.Add(item + suffix);
